...
async function getMeta(package) {
    try {
        ...
        if (!body.data || !body.data.length) {
            console.log(`failed to get info from nuget registry for package "${package.name}"`);
            return;
        }
        ...
        console.warn(`failed to get info from nuget registry for package "${package.name}"`, err)
    }
}
